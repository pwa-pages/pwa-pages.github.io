...
for file in "$dir"main*.js "$dir"runtime*.js "$dir"scripts*.js "$dir"styles*.css; do
  if [[ -f "$file" ]]; then
    ...
    if [[ "$filename" == styles*.css ]]; then
      ...
      sed -i "s|{{styles.css}}|$filename|g" dist/rosen-web-component/browser/rosen-permits.html
      echo "Replaced {{styles.css}} with $filename in dist/rosen-web-component/browser/rosen-permits.html"
      ...
      sed -i "s|{{styles.css}}|$filename|g" dist/rosen-web-component/browser/rosen-multiple.html
      echo "Replaced {{styles.css}} with $filename in dist/rosen-web-component/browser/rosen-multiple.html"
    fi
