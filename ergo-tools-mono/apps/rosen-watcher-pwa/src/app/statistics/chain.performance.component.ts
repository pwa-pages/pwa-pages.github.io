...
import { ChainPerfChartDataSet } from '@ergo-tools/service';
import { ChainChartPerformance } from '@ergo-tools/service';
  ...
  implements OnInit
{
  ...
  updateChart(): void {
    ...
    if (!this.performanceChart) {
      this.performanceChart = this.chartService.createChainPerformanceChart(
        dataSet,
        canvas,
      );
    } else {
      ...
      this.performanceChart.data.datasets[0].backgroundColor =
        this.chartService.chartColors;
