...
const chainTypeTokens = Object.fromEntries(Object.values(ChainType).map((chain) => chain === ChainType.Runes ? [chain, `rspv3BitcoinRunesRWT`] : [chain, `rspv3${chain}RWT`]));
const chainTypeWatcherIdentifier = Object.fromEntries(Object.values(ChainType).map((chain) => chain === ChainType.Runes ? [chain, `rspv3BitcoinRunesAWC`] : [chain, `rspv3${chain}AWC`]));
const rwtTokenIds = {
  ...
  async getWatcherInputs() {
    ...
    try {
      ...
      filteredInputs.forEach((input) => {
        input.assets = input.assets.filter((asset) => asset.amount < 5e6 && (asset.tokenId == rs_RSNTokenId || asset.tokenId == rs_eRSNTokenId)).map((asset_1) => {
          return asset_1;
        });
      });
...
//# debugId=3a7a2dbc-efdf-583a-a0a8-45a2dfda11e5
