...
class DownloadService {
  ...
  async downloadForAddresses() {
    ...
    try {
      ...
      const downloadPromises = addresses.map(
        async (addressObj) => {
  ...
  async downloadAllForAddress(address, offset, db, useNode, callback) {
    ...
    try {
      ...
      if (this.dataService.getMaxDownloadDateDifference() > (/* @__PURE__ */ new Date()).getTime() - new Date(
  ...
  async setDownloadStatus(address, status, db) {
    let dbStatus = await this.getDownloadStatus(
      address,
      db
    );
    if (!dbStatus) {
