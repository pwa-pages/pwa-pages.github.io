...
// Fetch Transactions
async function fetchTransactions(url: string): Promise<Transaction[]> {
  try {
...
async function addData(
  ...
  return new Promise((resolve, reject) => {
    ...
    const putRequests = transactions.flatMap((item) =>
...
async function getData(storeName: string, db: IDBDatabase): Promise<Address[]> {
  return new Promise((resolve, reject) => {
...
async function downloadForAddresses(db: IDBDatabase): Promise<void> {
  try {
    const addresses: Address[] = await getData(addressDataStoreName, db); // Fetch all addresses from the IndexedDB
