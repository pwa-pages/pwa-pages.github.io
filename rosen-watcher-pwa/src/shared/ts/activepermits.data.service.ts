...
class ActivePermitsDataService extends DataService<PermitTx> {
  ...
  async saveOpenBoxes(address: string, openBoxesJson: string, db: IDBDatabase): Promise<void> {
    return new Promise((resolve, reject) => {
      const transaction: IDBTransaction = db.transaction([rs_OpenBoxesStoreName], 'readwrite');
      const objectStore: IDBObjectStore = transaction.objectStore(rs_OpenBoxesStoreName);

      const boxes: OpenBoxes = { address: address, openBoxesJson: openBoxesJson };
  ...
  async addData(address: string, transactions: TransactionItem[], db: IDBDatabase): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const transaction: IDBTransaction = db.transaction([rs_ActivePermitTxStoreName], 'readwrite');
