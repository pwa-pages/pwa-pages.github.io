    ...
    return new Promise((resolve, reject) => {
      ...
      transactions.forEach((item) => {
        ...
        const eRSNTotal = item.outputs.reduce((total, output) => {
          ...
          return total + assets.reduce((acc, asset) => acc + asset.amount / Math.pow(10, asset.decimals), 0);
        }, 0);
  ...
  async getWatcherInputs() {
    ...
    try {
      ...
      filteredInputs.forEach((input) => {
        input.assets = input.assets.filter((asset) => asset.name === "RSN" || asset.name === "eRSN").map((asset_1) => {
          return asset_1;
        });
      });
...
class ProcessEventService {
  ...
  async processEvent(event) {
    if (event.type === "StatisticsScreenLoaded" || event.type === "PerformanceScreenLoaded" || event.type === "RequestInputsDownload") {
