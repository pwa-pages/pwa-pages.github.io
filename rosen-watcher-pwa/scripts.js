...
const chainTypeTokens = Object.fromEntries(Object.values(ChainType).map((chain) => chain === ChainType.Runes ? [chain, `rspv2BitcoinRunesRWT`] : [chain, `rspv2${chain}RWT`]));
const chainTypeWatcherIdentifier = Object.fromEntries(Object.values(ChainType).map((chain) => chain === ChainType.Runes ? [chain, `rspv2BitcoinRunesAWC`] : [chain, `rspv2${chain}AWC`]));
const rwtTokenIds = {
  ...
  "8bde33c8654ddb525b4db4842dd959e592c3847eabe40383af09a76be36379c4": ChainType.Runes
};
...
class StorageService {
  ...
  getStoreCache(storeName) {
    ...
    if (!sc) {
      sc = { byId: /* @__PURE__ */ new Map(), loaded: false };
  ...
  getKey(keyPath, item) {
    if (Array.isArray(keyPath)) {
      ...
      return key.every((k) => k !== void 0) ? key : void 0;
    }
    return item[keyPath];
  }
  // Load cache from DB if not loaded
  async ensureCache(storeName) {
    const storeCache = this.getStoreCache(storeName);
    if (!storeCache.loaded) {
      ...
      for (const item of allItems) {
        const key = keyPath ? this.getKey(keyPath, item) : void 0;
        if (key !== void 0)
  ...
  async getData(storeName) {
    const storeCache = await this.ensureCache(storeName);
    return Array.from(storeCache.byId.values());
  }
  async getDataById(storeName, id) {
    const storeCache = await this.ensureCache(storeName);
    return storeCache.byId.get(id) ?? null;
  }
