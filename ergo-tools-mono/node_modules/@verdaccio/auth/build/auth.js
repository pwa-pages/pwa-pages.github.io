...
class Auth {
  ...
  async loadPlugin() {
    ...
    }, this.options.legacyMergeConfigs, this.config?.server?.pluginPrefix ?? _core.PLUGIN_PREFIX, _core.PLUGIN_CATEGORY.AUTHENTICATION);
  }
  _applyDefaultPlugins() {
    // TODO: rename to applyFallbackPluginMethods
    this.plugins.push((0, _utils.getDefaultPlugins)(this.logger));
  }
  ...
  allow_access({
    ...
    const pkg = Object.assign({}, pkgAllowAccess, _core.authUtils.getMatchedPackagesSpec(packageName, this.config.packages));
  ...
  handleJWTAPIMiddleware(req, security, secret, authorization, next) {
    ...
    const {
      scheme,
      token
    } = (0, _utils.parseAuthTokenHeader)(authorization);
    if (scheme.toUpperCase() === _core.TOKEN_BASIC.toUpperCase()) {
      ...
      const credentials = (0, _utils.getMiddlewareCredentials)(security, secret, authorization);
      if (credentials) {
  ...
  webUIJWTmiddleware() {
    return (req, res, _next) => {
      ...
      let credentials;
      try {
        credentials = (0, _utils.verifyJWTPayload)(token, this.config.secret, this.config.security);
      } catch (err) {
