...
class ChainPerformanceDataService extends DataService {
  ...
  async getExistingData(transaction) {
    return new Promise((resolve, reject) => {
      const dbTtransaction = this.db.transaction(
        [rs_PerfTxStoreName],
        "readonly"
      );
      ...
      transactions.forEach((item) => {
        ...
        const eRSNTotal = item.outputs.reduce((total, output) => {
          ...
          const assets = output.assets.filter(
            (a) => a.tokenId === rs_eRSNTokenId && Object.values(rewardAddresses).includes(output.address)
          );
  ...
  async getPerfTxs() {
    ...
    try {
      ...
      perfTxs = perfTxs.filter(
        (p) => this.getMaxDownloadDateDifference() > (/* @__PURE__ */ new Date()).getTime() - new Date(p.timestamp).getTime()
      );
