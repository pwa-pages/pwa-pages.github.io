...
export const createHeader = ({ user, onLogout, onLogin, onCreateAccount }) => {
  ...
  if (user) {
    ...
    account.appendChild(createStatistics({ size: 'small', label: 'Log out', onClick: onLogout }));
  } else {
    account.appendChild(createStatistics({ size: 'small', label: 'Log in', onClick: onLogin }));
    account.appendChild(
      createStatistics({
        size: 'small',
        label: 'Sign up',
