...
for file in "$dir"main*.js "$dir"runtime*.js "$dir"styles*.css; do
  if [[ -f "$file" ]]; then
    ...
    if [[ "$filename" == main*.js ]]; then
      ...
      echo "Replaced {{main.js}} with $filename in dist/rosen-watchers/rosen_chain-performance.html"
    elif [[ "$filename" == runtime*.js ]]; then
      sed -i "s|{{runtime.js}}|$filename|g" dist/rosen-watchers/rosen_chain-performance.html
      echo "Replaced {{runtime.js}} with $filename in dist/rosen-watchers/rosen_chain-performance.html"
    elif [[ "$filename" == styles*.css ]]; then
      sed -i "s|{{styles.css}}|$filename|g" dist/rosen-watchers/rosen_chain-performance.html
      echo "Replaced {{styles.css}} with $filename in dist/rosen-watchers/rosen_chain-performance.html"
    fi
  fi
done
