...
class ChainPerformanceDataService extends DataService<PerfTx> {
  override async getExistingData(
    ...
    return new Promise((resolve, reject) => {
      const dbTtransaction: IDBTransaction = this.db.transaction(
        [rs_PerfTxStoreName],
        "readonly",
      );
      const objectStore: IDBObjectStore =
        dbTtransaction.objectStore(rs_PerfTxStoreName);
  ...
  override async addData(
    ...
    return new Promise((resolve, reject) => {
      ...
      transactions.forEach((item: TransactionItem) => {
        ...
        const eRSNTotal = item.outputs.reduce((total, output) => {
          ...
          return (
            total +
            assets.reduce(
              (acc, asset) => acc + asset.amount / Math.pow(10, asset.decimals),
              0,
            )
          );
        }, 0);
      ...
      const objectStore: IDBObjectStore =
        transaction.objectStore(rs_PerfTxStoreName);
