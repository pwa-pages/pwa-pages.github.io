...
class ActivePermitsDataService extends DataService {
  ...
  async getWatcherPermits() {
    ...
    try {
      ...
      permits.forEach((permit) => {
        permit.assets = permit.assets.filter(
          (asset) => asset.tokenId != null && asset.tokenId in rwtTokenIds
        ).map((asset_1) => {
          return asset_1;
        });
      });
  ...
  async getOpenBoxesMap(db) {
    ...
    for (const [, address] of Object.entries(permitBulkAddresses)) {
      if (address) {
        openBoxesMap[address] = await new Promise(
          (resolve, reject) => {
  ...
  async getAdressActivePermits(addresses = null) {
    ...
    if (addresses != null && addresses.length > 0) {
      addressPermits = permits.filter(
        (info) => addresses.some((addr) => addr === info.address)
      );
    }
    ...
    for (const permit of addressPermits) {
      let outputs = (permitsByTxId[permit.transactionId] ?? []).filter(
        (o) => Object.values(permitTriggerAddresses).some(
      ...
      for (const output of outputs) {
        ...
        if (cnt.length >= 2) {
          foundResolved = true;
          for (const p of cnt) {
            let txs = permitsByTxId[p.transactionId]?.filter(
              (t) => Object.values(permitBulkAddresses).includes(t.address)
            ) ?? [];
            await Promise.all(
              txs.map(async (t) => {
