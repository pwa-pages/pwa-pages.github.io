  ...
  "dist/service/commonjs/src/lib/process.event.service.js"() {
    var ProcessEventService2 = class {
      ...
      async processEvent(event) {
        if (event.type === "MainScreenLoaded" || event.type === "EventsScreenLoaded" || event.type === "MyAntichessScreenLoaded" || event.type === "RequestInputsDownload") {
          ...
          } else if (event.type === "MainScreenLoaded") {
            await this.processMainScreenLoaded(dataService, downloadService);
          } else if (event.type === "MyAntichessScreenLoaded") {
            await this.processMyAntichessScreenLoaded(event, myWatcherDataService, downloadMyWatchersService, activePermitsDataService, downloadActivePermitsService);
          } else if (event.type === "EventsScreenLoaded") {
            await this.processEventsScreenLoaded(chainPerformanceDataService, downloadPerfService);
          }
        }
      }
      async processEventsScreenLoaded(chainPerformanceDataService, downloadPerfService) {
        console.log("Rosen service worker received EventsScreenLoaded");
        try {
      ...
      async processMyAntichessScreenLoaded(event, myWatcherDataService, downloadMyWatchersService, activePermitsDataService, downloadActivePermitsService) {
        const addresses = event.data.addresses;
        console.log("Rosen service worker received MyAntichessScreenLoaded initiating syncing of data by downloading from blockchain");
        try {
      ...
      async processMainScreenLoaded(dataService, downloadService) {
        console.log("Rosen service worker received MainScreenLoaded initiating syncing of data by downloading from blockchain");
        try {
