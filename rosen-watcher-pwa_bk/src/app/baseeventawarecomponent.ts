...
export class BaseEventAwareComponent implements OnDestroy {
  ...
  async subscribeToEvent<T>(eventType: EventType, callback: (...args: T[]) => void) {
    ...
    await this.eventService.subscribeToEvent(eventType, eventCallBack, this.componentId);
  }

  async subscribeToEvents(eventTypes: EventType[], callback: () => void) {
    eventTypes.forEach(async (eventType) => {
      await this.eventService.subscribeToEvent(eventType, callback, this.componentId);
    });
  }
