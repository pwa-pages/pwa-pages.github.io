  ...
  async downloadAllForAddress(
    ...
    try {
      ...
      await this.downloadAllForAddress(
        address,
        offset + rs_FullDownloadsBatchSize,
        db,
        profile,
      );
    } catch (e) {
  ...
  */
  ...
  async getDownloadStatus(address: string, db: IDBDatabase): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () =>
        resolve((request.result as DownloadStatus)?.status || "false");
  ...
  async downloadForAddress(
    ...
    try {
      ...
      console.log(
        `Processing initial download(size = ${rs_InitialNDownloads}) for: ${address}`,
      );
        ...
        for (
          ...
          for (const input of item.inputs) {
            if (
              input.boxId &&
              halfBoxId === "" &&
              (await this.dataService.getDataByBoxId(
                input.boxId,
                address,
                db,
              )) &&
      ...
      const boxData = await this.dataService.getDataByBoxId(
        halfBoxId,
        address,
        db,
      );
